...
%folder_name = 'Marcelo'; %Escolher o nome da pasta para salvar
...
fprintf('\n')
prompt4 = 'Digite o nome da pasta: ';
folder_name = input(prompt4,'s');
destdirectory = folder_name;
mkdir(destdirectory);
cd(destdirectory)
...
[AQ, byteOrder, storageFormat] = btkReadAcquisition([path file num2str(DIR) ext]);
...
cutoff = 0.7;
sx = size(angle_matrix_raw);
for i = 1:sx(2)
    for j = 1:sx(2)
    c = corrcoef(angle_matrix(:,i),angle_matrix(:,j));
...
for i = 1:length(angle_names)
    for j =1:length(eixos)
    var_name{k} = [angle_names{i} '_' eixos{j}];
    k = k +1;
    end
end
f2=figure;
